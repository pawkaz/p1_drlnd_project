...
### Train model
To train agents, run in the terminal:
...
Notice that, you will be notified when the enviornment is solved. The best models are saved according to the mean score over 100 consecutive episodes in the files ```checkpoint_actor_A.pt```, ```checkpoint_critic_A.pt```, ```checkpoint_actor_B.pt```, ```checkpoint_critic_B.pt```.
